...
public:
	...
	void AutonomousInit(void)
	{
		...
		auto_drive_speed = auto_split ? 0.37 : 0.4;
	...
	void joystick_drive(Joystick& drivestick)
	{
		...
		turn_adjust_value = (fabs ((3.0 * pow((1 - fabs(joy_velocity)), 4) + 0.5)));
	...
	void pickup_state_machine(bool auto_execute = false)
	{
		switch(pickup_state)
		{
			...
			case pickup_start:
				...
				set_position(pivot, pivot_floor_val, 0.4, -0.4); // @@@ Change to 0.4 for real bot
			...
			case pickup_pivoting_up:
				if (at_position(pivot, 3))
				{
					if (at_position(gripper, 1))
						...
						if (stalled(gripper))
						{
							set_position(gripper, gripper_open_val, 0.27, -0.27);
							pickup_state = pickup_parked;
						}
				}
			break;
	...
	void drag_tube(void)
	{
		switch (drag_state)
		{
			...
			case 1: // Initiate pivoting down
				...
				set_position(pivot, pivot_floor_val, 0.4, -0.4); //@@@ set to 0.4 on real bot
				dly_tmr = 0.2;
				drag_state++;
			break;
			case 2: // Delay
	...
	void deploy_minibot()
	{
		...
		if (deploy_ok())
		{
			...
			if (launch_ready)
			{
				...
				if (at_position(mini) && 
				    ( button_pressed(stick_appendage, button_6) || button_pressed(stick_right, button_6) ))
				{
