...
//Cal this in Auto, Victors are messed up!
void Launcher_Autonomous(void){
    Launcher_State_Machine();
  //  printf("Motor: %3d \r\n",motor_l);
    #ifdef PRACTICE_BOT
        LAUNCH_MOTOR = (unsigned char) LimitChar(motor_l);
         GRIPPER_MOTOR_A = 127-15; //stupid victor calibration is wrong!
        GRIPPER_MOTOR_B = 127+15; //stupid victor calibration is wrong!
    #endif
    #ifndef PRACTICE_BOT
        LAUNCH_MOTOR = (unsigned char) LimitChar(~motor_l);
         GRIPPER_MOTOR_A = 127+15; //stupid victor calibration is wrong!
...
//Call this to operate the launcher
...
//Abstract the gripper control
void SetGripper(int in){
    motor_g = in;
}


//State machine to handle launcher semi autonomously
void Launcher_State_Machine(void){
    ...
    switch(state)
    {
        ...
        case FINDING_TOP:
            ...
            motor_l = -8; //elastic pulls a bit too hard, back drive
            brake = OPEN_B;
        ...
        case FLOOR:
            ...
            if(pot_delta > 20){ //positive 20 = launcher moving up
                state = IDLE ;
            }
        ...
        case ARMED:
            ...
            if(OP_LAUNCH && !tilter){
				//set a flag saying its OK to launch
				//in case state becomes LAUNCHING by some unexpected behavior
				//i like redundancy
